...
import { ensureMediaDirectory } from './services/media-service';
import { nowIso } from './utils/time';
import { menuSeeds } from './data/menu-seeds';
...
const ensureSeedMedia = () => {
  ...
  const mirroredDirectories = new Set([path.resolve(env.MEDIA_UPLOAD_DIR), path.resolve(process.cwd(), 'public', 'uploads')]);
  const sourceEntries = fs.existsSync(siteMediaDirectory) ? fs.readdirSync(siteMediaDirectory, { withFileTypes: true }) : [];

  for (const entry of sourceEntries) {
    if (!entry.isFile()) continue;

    const source = path.join(siteMediaDirectory, entry.name);
...
const createMediaAsset = (filename: string, collectionSlug: string, hrAlt: string, enAlt: string) => {
  const sourcePath = path.join(siteMediaDirectory, filename);
  const fileStats = fs.existsSync(sourcePath) ? fs.statSync(sourcePath) : null;
  const mimeType = inferMimeTypeFromFilename(filename);
  const existing = db.prepare('SELECT id FROM media_assets WHERE filename = ?').get(filename) as { id: number } | undefined;
  const assetId =
    existing?.id ??
    Number(
      db
        ...
        .run(filename, filename, sourcePath, mimeType, fileStats?.size ?? 0, JSON.stringify([collectionSlug, 'premium']), nowIso(), nowIso()).lastInsertRowid
    );
  ...
  db.prepare(
    `UPDATE media_assets
     ...
     WHERE id = ?`
  ).run(filename, sourcePath, mimeType, fileStats?.size ?? 0, JSON.stringify([collectionSlug, 'premium']), nowIso(), assetId);
  ...
  for (const [locale, alt] of [
    ...
    db.prepare(
      `INSERT INTO media_asset_localizations (media_asset_id, locale, alt_text, caption, created_at, updated_at)
       VALUES (?, ?, ?, '', ?, ?)
       ON CONFLICT(media_asset_id, locale) DO UPDATE SET alt_text = excluded.alt_text, updated_at = excluded.updated_at`
    ).run(assetId, locale, alt, nowIso(), nowIso());
  }

  const collection = db.prepare('SELECT id FROM media_collections WHERE slug = ?').get(collectionSlug) as { id: number } | undefined;
  if (collection) {
    db.prepare(
      `INSERT INTO media_collection_items (collection_id, media_asset_id, sort_order, created_at)
       VALUES (?, ?, ?, ?)
       ON CONFLICT(collection_id, media_asset_id) DO NOTHING`
    ).run(collection.id, assetId, 0, nowIso());
  }
...
export const seedDatabase = () => {
  if (!env.ENABLE_SEED_CONTENT) return;
  ensureSeedMedia();

  runInTransaction(() => {
    const adminExists = db.prepare('SELECT id FROM admins LIMIT 1').get() as { id: number } | undefined;
    if (!adminExists) {
      if (!env.ADMIN_BOOTSTRAP_EMAIL || !env.ADMIN_BOOTSTRAP_PASSWORD) {
        logger.warn('admin_bootstrap_skipped_missing_credentials');
      } else {
        db.prepare(
          `INSERT INTO admins (email, password_hash, display_name, role, created_at, updated_at)
           VALUES (?, ?, ?, 'staff', ?, ?)`
        ).run(env.ADMIN_BOOTSTRAP_EMAIL, bcrypt.hashSync(env.ADMIN_BOOTSTRAP_PASSWORD, 12), 'Nautica Admin', nowIso(), nowIso());
      }
    }

    db.prepare(
      `INSERT INTO business_settings (id, business_name, timezone, phone, whatsapp_phone, email, address, city, created_at, updated_at)
       VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    ).run('Nautica', env.BUSINESS_TIMEZONE, '+385 99 785 6785', '+385 99 785 6785', 'trebocconi@trebocconi.com', 'DONJA RIVA 55', '22212 Tribunj, Croatia', nowIso(), nowIso());
    ...
    for (const [weekday, opensAt, closesAt, isClosed] of [
      ...
      db.prepare(
        `INSERT INTO business_hours (weekday, opens_at, closes_at, is_closed, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)
         ON CONFLICT(weekday) DO UPDATE SET
           ...
           updated_at = excluded.updated_at`
      ).run(weekday, opensAt, closesAt, isClosed, nowIso(), nowIso());
    }
    ...
    for (const [scope, contentKey, locale, value] of localizedContentRows) {
      ...
      ).run(scope, contentKey, locale, value, nowIso(), nowIso());
    }

    for (const seed of faqSeeds) {
      const existing = db.prepare('SELECT id FROM faqs WHERE category = ? AND sort_order = ?').get(seed.category, seed.sortOrder) as { id: number } | undefined;
      const faqId =
        existing?.id ??
        Number(
          db
            ...
            .run(seed.active, seed.category, seed.sortOrder, nowIso(), nowIso()).lastInsertRowid
        );
      ...
      db.prepare(
        `UPDATE faqs
         ...
         WHERE id = ?`
      ).run(seed.active, seed.category, seed.sortOrder, nowIso(), faqId);
      ...
      for (const [locale, question, answer] of [
        ...
        ).run(faqId, locale, question, answer, nowIso(), nowIso());
      }
    }
    ...
    for (const [slug, name, sortOrder] of [
      ...
      db.prepare(
        `INSERT INTO media_collections (slug, name, sort_order, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?)
         ON CONFLICT(slug) DO UPDATE SET name = excluded.name, sort_order = excluded.sort_order, updated_at = excluded.updated_at`
      ).run(slug, name, sortOrder, nowIso(), nowIso());
    }
    ...
    const resolveMenuItemMediaId = (category: (typeof menuSeeds)[number]['category']) => {
      switch (category) {
        case 'hot_beverages':
    ...
    db.prepare('DELETE FROM menu_item_localizations').run();
    db.prepare('DELETE FROM menu_items').run();

    for (const seed of menuSeeds) {
      const existing = db.prepare('SELECT id FROM menu_items WHERE category = ? AND sort_order = ?').get(seed.category, seed.sortOrder) as { id: number } | undefined;
      const menuItemId =
        existing?.id ??
        Number(
          db
            ...
            .run(
              ...
              seed.priceLabel,
              seed.sortOrder,
              seed.secondaryPriceLabel,
              seed.featured,
              JSON.stringify(seed.labels),
              JSON.stringify(seed.allergens),
              resolveMenuItemMediaId(seed.category),
              seed.bookSection,
              seed.spreadStyle,
              nowIso(),
              nowIso()
      ...
      for (const [locale, name, description] of [
        ...
        ).run(menuItemId, locale, name, description, nowIso(), nowIso());
      }
    }

    for (const seed of eventSeeds) {
      const existing = db.prepare('SELECT id FROM events WHERE slug = ?').get(seed.slug) as { id: number } | undefined;
      ...
      const eventId =
        ...
        Number(
          db
            ...
            .run(seed.slug, seed.status, seed.featured, seed.capacity, seed.waitlistEnabled, seed.startsAt, seed.endsAt, seed.imageUrl, seed.category, env.BUSINESS_TIMEZONE, posterMediaId, seed.reservationMode, seed.priceLabel, nowIso(), nowIso()).lastInsertRowid
        );
      ...
      db.prepare(
        `UPDATE events
         ...
         WHERE id = ?`
      ).run(seed.category, env.BUSINESS_TIMEZONE, posterMediaId, seed.reservationMode, seed.priceLabel, seed.ticketUrl, nowIso(), eventId);
      ...
      for (const [locale, title, teaser, description] of [
        ...
        ).run(eventId, locale, title, teaser, description, nowIso(), nowIso());
      }

      db.prepare('DELETE FROM event_tags WHERE event_id = ?').run(eventId);
      seed.tags.forEach((tag) => {
        db.prepare('INSERT OR IGNORE INTO event_tags (event_id, tag, created_at) VALUES (?, ?, ?)').run(eventId, tag, nowIso());
      });
      ...
      db.prepare('DELETE FROM event_media WHERE event_id = ?').run(eventId);
      galleryMedia.forEach((mediaId, index) => {
        if (!mediaId) return;
        db.prepare(
          `INSERT OR IGNORE INTO event_media (event_id, media_asset_id, sort_order, created_at)
           VALUES (?, ?, ?, ?)`
        ).run(eventId, mediaId, index, nowIso());
      });
    }
    ...
    for (const [moduleKey, enabled, sortOrder, settings] of homepageModules) {
      db.prepare(
        `INSERT INTO homepage_modules (module_key, enabled, sort_order, settings_json, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)
         ON CONFLICT(module_key) DO UPDATE SET enabled = excluded.enabled, sort_order = excluded.sort_order, settings_json = excluded.settings_json, updated_at = excluded.updated_at`
      ).run(moduleKey, enabled, sortOrder, JSON.stringify(settings), nowIso(), nowIso());
    }

    for (const seed of announcementSeeds) {
      const eventId = seed.eventSlug
        ? (db.prepare('SELECT id FROM events WHERE slug = ?').get(seed.eventSlug) as { id: number } | undefined)?.id ?? null
        : null;
      const existing = db.prepare('SELECT id FROM announcements WHERE persistent_dismissal_key IS ? AND priority = ?').get(seed.persistentDismissalKey, seed.priority) as { id: number } | undefined;
      const announcementId =
        ...
        Number(
          db
            ...
            .run(seed.status, seed.variant, seed.priority, seed.sortOrder, seed.dismissible, seed.persistentDismissalKey, seed.ctaUrl, eventId, seed.reservationIntent, seed.startsAt, seed.endsAt, nowIso(), nowIso()).lastInsertRowid
        );
      ...
      for (const [locale, copy] of [
        ...
        ).run(announcementId, locale, copy.title, copy.body, copy.ctaLabel, nowIso(), nowIso());
      }
    }

    const glimpseExists = db.prepare('SELECT id FROM glimpse_groups LIMIT 1').get() as { id: number } | undefined;
    if (!glimpseExists) {
      const groupId = Number(
        db
          ...
          .run(mediaIds.get('hero-sunset.jpg') ?? null, nowIso(), nowIso()).lastInsertRowid
      );
      ...
      for (const [locale, label, title] of [
        ...
        db.prepare(
          `INSERT INTO glimpse_group_localizations (group_id, locale, label, title, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?)`
        ).run(groupId, locale, label, title, nowIso(), nowIso());
      }
      ...
      slides.forEach(([filename, hrHeadline, hrBody, enHeadline, enBody, ctaUrl], index) => {
        const slideId = Number(
          db
            ...
            .run(groupId, mediaIds.get(filename) ?? null, index + 1, ctaUrl, nowIso(), nowIso()).lastInsertRowid
        );
        ...
        for (const [locale, headline, body, ctaLabel] of [
          ...
          db.prepare(
            `INSERT INTO glimpse_slide_localizations (slide_id, locale, headline, body, cta_label, created_at, updated_at)
             VALUES (?, ?, ?, ?, ?, ?, ?)`
          ).run(slideId, locale, headline, body, ctaLabel, nowIso(), nowIso());
        }
      });
    }
  });
